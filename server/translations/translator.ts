...
function translateExcerpt(excerpt: string): string {
  ...
  // Se não estiver no mapeamento, retorna o original
  // Em um ambiente de produção, aqui seria o local para integrar uma API de tradução
  return excerpt;
}
...
export async function migrateTranslations() {
  ...
  try {
    ...
    console.log(`Encontradas ${allCategories.length} categorias para traduzir.`);
    ...
    for (const category of allCategories) {
      try {
        ...
        console.log(`Categoria traduzida com sucesso: ${category.name} -> ${translateCategory(category.name)}`);
      } catch (error) {
        console.error(`Erro ao traduzir categoria ${category.name}:`, error);
      }
    }
    ...
    console.log(`Encontradas ${allStories.length} estórias para traduzir.`);
    ...
    for (const story of allStories) {
      try {
        ...
        console.log(`Estória traduzida com sucesso: ${story.title} -> ${translateTitle(story.title)}`);
      } catch (error) {
        console.error(`Erro ao traduzir estória ${story.title}:`, error);
      }
    }
    ...
    console.log(`Encontrados ${allTestimonials.length} testemunhos para traduzir.`);
    ...
    for (const testimonial of allTestimonials) {
      try {
        ...
        console.log(`Testemunho traduzido com sucesso: ${testimonial.name}`);
      } catch (error) {
        console.error(`Erro ao traduzir testemunho ${testimonial.name}:`, error);
      }
    }
    ...
    console.error("Erro durante a migração de traduções:", error);
    throw error;
  }
}
